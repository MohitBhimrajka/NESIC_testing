...
# Defined as regular strings

# Updated: Added explicit checks for financial data citation verification.
ADDITIONAL_REFINED_INSTRUCTIONS = textwrap.dedent("""\
    ...
    *   **Mandatory Self-Check Before Final Output:**
        - Before producing the final answer, confirm:
            ...
            8. Strictly reference only the exact named company; do not include similarly named entities. Check clarity on parent vs. subsidiary data reporting.
...
# Updated: Changed Quantity instruction to list *all* verifiable sources used.
FINAL_SOURCE_LIST_INSTRUCTIONS_TEMPLATE = textwrap.dedent("""\
    ...
    *   The "**Sources**" section must appear only once, at the end of the entire response.
    """) # Note: {language} placeholder remains literal here, will be replaced by main f-string

# (Other general instruction blocks like HANDLING_MISSING_INFO_INSTRUCTION, RESEARCH_DEPTH_INSTRUCTION, ANALYSIS_SYNTHESIS_INSTRUCTION, INLINE_CITATION_INSTRUCTION, SPECIFICITY_INSTRUCTION, AUDIENCE_CONTEXT_REMINDER, get_language_instruction, BASE_FORMATTING_INSTRUCTIONS, FINAL_REVIEW_INSTRUCTION, COMPLETION_INSTRUCTION_TEMPLATE remain the same as in the previous version, as they already incorporate detail/analysis enhancements)
# Updated: Enhanced cross-document search guidance and group structure handling.
...
# Updated: Added strict recency, cross-document search, data verification, group structure handling, and chart notation. Emphasized primary sources more strongly.
RESEARCH_DEPTH_INSTRUCTION = textwrap.dedent("""\
    *   **Research Depth & Source Prioritization:**
        ...
        *   **Group Structure Handling:** Clearly identify if data refers to the consolidated parent group or a specific target subsidiary. Actively search for subsidiary-specific sections, appendices, or footnotes within parent company reports. Acknowledge (internally for decision-making) potential data limitations for non-publicly listed subsidiaries.
...
# Updated: Enhanced requirements for explaining 'why', quantification, comparison, causal linkage, and adding DX implications.
...
# Updated: Reinforced table data integrity checks and quantitative summaries.
...
FINAL_REVIEW_INSTRUCTION = textwrap.dedent("""\
    *   **Internal Final Review:** Before generating the 'Sources' list, review your generated response for:
        ...
        *   **Single-Entity & Group Structure Clarity:**
            ...
            * Clarity maintained between parent group data and specific subsidiary data where applicable.
...
COMPLETION_INSTRUCTION_TEMPLATE = textwrap.dedent("""\
    **Output Completion Requirements:**
    ...
    6. Strictly focus on the exact named company (no confusion with similarly named entities). Verify parent vs. subsidiary context where needed.
""")

# --- Prompt Generating Functions ---

# Basic Prompt (No major changes needed based on specific financial feedback)
...
Target Audience Context: The final research output is intended for review and strategic planning by a **Japanese company**. Present the information clearly and accurately with granular details and actionable insights. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}

{get_language_instruction(language)}

Research Requirements:
Conduct in-depth research using {company_name}'s official sources. Perform exhaustive checks across multiple primary sources before omitting any requested information silently. Every factual claim, data point, and summary must include an inline citation in the format [SSX]. Provide specific dates or reporting periods (e.g., "as of 2024-03-31", "for FY2023"). Ensure every claim is grounded by a verifiable Vertex AI grounding URL referenced back in the final Sources list. Use the absolute latest available official information.
...
## 8. General Discussion:
    ...
    *   **Synthesize** the key findings exclusively from Sections 1-7, explicitly linking analysis (e.g., "The organizational structure described in section 4 [SSX] supports the strategic focus mentioned by the CEO [SSY]...") and ensuring every claim is supported by an inline citation. Incorporate key quantitative points.
...
*   **Accuracy:** All information must be factually correct, current, and verifiable against grounded sources. Specify currency and reporting periods for all monetary data. Omit unverified data silently after exhaustive search. Verify management lists against latest website data.
...
{FINAL_SOURCE_LIST_INSTRUCTIONS_TEMPLATE.format(language=language)}
{BASE_FORMATTING_INSTRUCTIONS.format(language=language)}
"""
    return prompt

# Updated: Financial prompt with reinforced sourcing rules, calculation citation guidance, and analytical depth.
def get_financial_prompt(company_name: str, language: str = "Japanese"):
    ...
    *   **Data Sparsity Acknowledgement (Internal):** For non-listed subsidiaries or complex groups, acknowledge internally that certain detailed metrics might be unavailable at the subsidiary level and analysis will rely on available consolidated or segment data.
    """)

    analytical_depth_instructions = textwrap.dedent("""\
    *   **Analytical Depth Requirements:**
        *   **Time-Series Trends:** For key metrics, identify and analyze growth/decline trends over the 3-year period. Quantify these trends (e.g., CAGR, YoY change) [SSX]. Explain the *drivers* behind these trends using management commentary or related data (e.g., cost structure changes impacting margins) [SSY].
        ...
        *   **Management Efficiency Evaluation:** Objectively evaluate management efficiency using relevant ratios (ROE, ROA, Margins, etc.) compared to past performance and targets [SSX].
        *   **Causal & Correlation Analysis:** Analyze potential correlations (e.g., sales vs. advertising costs, operating profit vs. personnel costs) based on reported data and management discussion [SSX]. Identify key drivers impacting profitability (e.g., "Which KPI is working for profit?" based on segment data or management statements) [SSY].
        *   **Identify Key Management Drivers:** Based on the analysis, highlight the primary levers management appears to be using or focusing on to influence financial performance [SSX].
    """)

    advanced_analysis_feasibility_note = textwrap.dedent("""\
    *   **Advanced Analysis (Feasibility Dependent):** If sufficient historical and competitive data is available and grounded, attempt the following:
        ...
        *   **Financial Soundness Risk Scoring:** (Conceptual) Briefly assess financial soundness based on key ratios (leverage, liquidity, profitability trends). *Do not create a numerical score unless a published methodology is cited [SSX].*
        *   **Scenario Analysis / Forecasting:** (Conceptual) Summarize any company-provided forecasts or scenarios (e.g., MTP targets, sensitivity analysis like FX impact mentioned in reports [SSX]). *Do not perform independent forecasting.* Briefly describe forecasting models mentioned in sources if any [SSY].
    """)

    prompt = f"""
Comprehensive Strategic Financial Analysis of {company_name} (Last 3 Fiscal Years)

Objective: Deliver a complete, analytically rich, and meticulously sourced financial profile of **{company_name}** using the last three full fiscal years. Combine traditional financial metrics with analysis of profitability, cost structure, cash flow, investments, and contextual factors. Provide deep analysis explaining trends and drivers, requiring meticulous sourcing and in-depth analysis explaining the 'why' behind the numbers.

Target Audience Context: This analysis is for a **Japanese corporate strategy audience**. Use Japanese terminology when appropriate (e.g., "売上総利益" for Gross Profit) and ensure that all monetary values specify currency (e.g., JPY millions) and reporting period (e.g., "FY2023 ended March 31, 2024") with exact dates where available [SSX]. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}

{get_language_instruction(language)}

Research Requirements:
For each section, provide verifiable data with inline citations [SSX] and specific dates or reporting periods after conducting exhaustive research across multiple primary sources (including **footnotes**). **Every single financial figure MUST have a verifiable grounding URL citation [SSX] from this query.** Every claim must be traceable to a final source. Silently omit any data not found. Use **perfect Markdown tables** for financial data presentation, verifying data accuracy against sources. Use '-' for missing data points only if needed for table structure.
{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION}
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
{enhanced_financial_research_instructions}
{ANALYSIS_SYNTHESIS_INSTRUCTION} # Requesting deeper analysis beyond simple listing
{analytical_depth_instructions} # Explicit analytical tasks
{advanced_analysis_feasibility_note} # Optional advanced tasks

{ADDITIONAL_REFINED_INSTRUCTIONS}

## 1. Top Shareholders:
    *   List major shareholders (typically the top 5-10, with exact ownership percentages, reporting dates, and source references) in a **perfectly formatted Markdown table** [SSX]. Use '-' for missing data points only if needed for table structure.
...
## 2. Key Financial Metrics (3-Year Trend in a Table):
    ...
    *   **Analyze** key trends observed in the table (YoY changes, CAGR). Explain the *drivers* behind these trends based on source commentary [SSX]. Identify any standout performance aspects (positive or negative) [SSY].

## 3. Profitability Analysis (3-Year Trend):
    *   Analyze trends in Operating Margin and Net Income Margin in more detail (building on the table above). Explain the *drivers* behind these trends (e.g., cost variations, pricing power, product mix shifts, one-off items mentioned in reports) with specific evidence and inline citations [SSX]. Quantify changes YoY. Discuss the sustainability of current profitability levels [SSY].

## 4. Segment-Level Performance (if applicable, Last 3 Fiscal Years):
    *   If segment data is available (e.g., Mobility, Safety, Entertainment), present revenue, operating profit, and margin percentages for each segment in a **perfectly formatted Markdown table** (include currency and fiscal year, verify data) [SSX]. Use '-' for missing data points only if needed for table structure.
        ...
        | ...          | ...              |        |        |        |           |
    *   Analyze trends, growth drivers, and the relative contribution/profitability of each segment, citing specific figures [SSX]. Identify key profit-driving segments based on available data [SSY].

## 5. Cost Structure Analysis (3-Year Trend):
    *   Detail the composition and trends of major operating costs using data from financial statements [SSX]. Present in a **perfectly formatted Markdown table** if helpful and data is verifiable. Verify data accuracy. Use '-' for missing data points only if needed for table structure.
        ...
        |  - Personnel     |                |                   |                |                   |                |                   | [SSW]     |
        |  - Other SG&A    |                |                   |                |                   |                |                   | [SSV]     |
    *   Analyze drivers behind cost trends (e.g., raw material prices, personnel costs, restructuring effects) and comment on cost control effectiveness based on commentary in reports [SSX]. Look for fixed vs variable cost commentary if available [SSY].

## 6. Cash Flow Statement Analysis (3-Year Trend):
    *   Analyze trends in Operating Cash Flow (OCF). Explain key drivers, differentiating between changes in profit and changes in working capital components (receivables, payables, inventory) based on the cash flow statement details [SSX].
    *   Detail major Investing Cash Flow activities (e.g., CapEx, acquisitions) and Financing Cash Flow activities (e.g., debt issuance/repayment, dividends, share buybacks) with specific amounts (specify currency) and context [SSX].
    *   Calculate and analyze Free Cash Flow (FCF = OCF - CapEx) trend [SSX]. Cite base data sources [SSY, SSZ]. Comment on the company's capacity to fund operations, investments, and shareholder returns based on FCF generation [SSW].

## 7. Investment Activities (Last 3 Years):
    *   Describe major M&A deals (target, deal value if public, date, strategic rationale) [SSX].
    *   Analyze capital expenditure (CapEx) patterns (total amount, key areas like factories/equipment/software) [SSY].
    *   Detail any significant corporate venture capital (CVC) or R&D investments with specific amounts (specify currency and reporting period) and stated goals [SSZ].
    *   Analyze the strategic rationale and potential financial impact (if commented on by management) of these investments [SSX, SSY, SSZ].

## 8. Contextual Financial Factors:
    *   Identify significant one-time events (e.g., asset sales, restructuring charges, impairment losses, litigation settlements) reported in the last 3 years, specifying dates, financial impacts (gain/loss in specified currency), and source notes [SSX].
    *   Discuss any significant accounting standard changes that impacted reported figures during the period [SSY].
    *   Mention any key external economic or regulatory factors explicitly cited by management as impacting financial performance [SSZ].
    *   Critically analyze the quality and sustainability of reported earnings, considering the impact of one-time items and accounting choices noted [SSX, SSY, SSZ].

## 9. Credit Ratings & Financial Health (if available):
    *   List current and historical credit ratings from major agencies (e.g., S&P, Moody's, Fitch, R&I, JCR) with reporting dates [SSX].
    *   Summarize key highlights or concerns mentioned in the rating agencies' commentary [SSY].
    *   Analyze the implications of these ratings (or lack thereof) for the company's financial flexibility and cost of capital [SSX, SSY].

## General Discussion:
    *   Provide a concluding single paragraph (300-500 words) that synthesizes the findings exclusively from Sections 1-9. Explicitly connect the analysis (e.g., "The strong cash flow generation [SSX] supports the investment strategy outlined in Section 7 [SSY], despite the margin pressure noted in Section 3 [SSZ]..."). Explain *why* trends are occurring based on the analysis. Incorporate key quantitative results. Discuss implications for future financial performance and strategic options.
    *   Structure the discussion logically by starting with an overall assessment of financial health and performance trends, then discussing profitability drivers, cash flow adequacy, investment effectiveness, and concluding with an outlook (including strengths/weaknesses) tailored to a Japanese audience.
    *   Do not introduce any new factual claims that are not supported by previous sections and citations.

Source and Accuracy Requirements:
*   **Accuracy:** All information must be current and verifiable. Specify currency (e.g., JPY millions) and reporting period (e.g., FY2023) for every monetary value. Silently omit unverified data after exhaustive search. Verify table data meticulously. **Every financial figure must have a grounding URL citation [SSX].**
...
*   **Source Quality:** Rely primarily on official company sources (Financial Statements, Footnotes, Tanshin, IR Presentations, Annual Reports). Secondary sources may be used sparingly for context (like ratings) and must be clearly cited, verified, and grounded.
...
# (Competitive Landscape, Management Strategy, Regulatory, Crisis, DX, Business Structure, Vision, Management Message, Account Strategy prompts remain the same as the previous version - the core financial sourcing fix is concentrated in get_financial_prompt and the general instruction blocks)
# Updated: Competitive Landscape prompt significantly expanded based on JAL feedback.
...
Target Audience Context: This output is for strategic review by a **Japanese company**. Ensure all analysis is supported by explicit inline citations [SSX] for {company_name}'s data/statements and [SSY, SSZ] for grounded industry/competitor data. Clearly attribute synthesized points. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}
...
{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION} # Focus on {company_name}'s view + grounded competitor/industry data
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
...
### 1. Industry Overview & Trends
    *   Describe the overall industry {company_name} operates in. Include market size and growth rate estimates if verifiable data is available [SSY].
...
### 2. Major Competitors Identification & Profiling
    *   Identify primary global and key regional competitors based on {company_name}'s official statements [SSX] or grounded third-party reports [SSY]. Provide specific names.
    *   Present competitor information in a **perfectly formatted Markdown table** where possible, clearly indicating source for each piece of data. Use '-' for missing data points only if needed for table structure. Verify data accuracy.
        | Competitor Name | Primary Business Area(s) of Overlap | Estimated Market Share (Market, Year) | Key Geographic Overlap | Recent Key Moves (Date) | Source(s) |
        |-----------------|-------------------------------------|---------------------------------------|------------------------|-------------------------|-----------|
...
### 4. {company_name}'s Detailed Profile (Competitive Lens)
    *   **Products and Services:**
        *   Describe main products/services and product line-up details [SSX].
    ...
    *   **Marketing and Sales Strategies:**
        *   Describe primary sales channels (e.g., direct, EC, distributors) [SSX].
        *   Outline promotion strategies mentioned (advertising focus, SNS campaigns, etc.) [SSY].
        *   Summarize reported brand image or perception [SSZ].
    ...
    *   **Technological and Development Capabilities:**
        *   List any claimed patents or unique technologies [SSX].
        *   Report R&D expenditure trends (absolute and % of revenue if available) [SSY].
        *   Identify key development bases or centers [SSZ].
        *   Detail significant external collaborations (universities, research institutions, other companies) mentioned [SSX].
    *   **Other Relevant Factors (if information available):**
...
### 5. {company_name}'s Competitive Strategy
    ...
    *   List specific initiatives or investments aimed at enhancing its competitive position (e.g., "Invested ¥XB in new R&D facility targeting Y technology [SSX]"). Include funding amounts and timelines if available [SSY].
...
### 6. General Discussion
    *   Provide a concluding single paragraph (300-500 words) that synthesizes the findings exclusively from Sections 1-5. Clearly link analytical statements using inline citations (e.g., "Given the industry trend towards X [SSY], {company_name}'s investment in Y technology [SSX] positions it well against Competitor A's recent moves [SSZ]..."). Evaluate the overall competitive strength and strategic effectiveness.
...
# Updated: Management Strategy prompt emphasizes extracting ALL MTP KPIs.
...
## 1. Management Strategy and Vision Alignment:
    ...
    *   Explain the core management philosophy, values, and strategic approach (e.g., "focus on organic growth through R&D [SSX]", "pursuit of operational excellence [SSY]") with examples, including specific dates or document references [SSZ].
    *   Identify key strategic pillars or themes (e.g., "Digital Transformation", "Sustainability", "Global Expansion") for the upcoming 3-5 years, explaining the rationale and objectives for each based on official statements [SSX, SSY].
    *   Describe any significant strategic shifts from previous plans (e.g., "pivot from hardware to software solutions announced in FY2022 [SSX]"), with supporting data and source references [SSY].

## 2. Current Mid-Term Business Plan (MTP) Overview:
    *   Identify the official name and exact time period of the current MTP (e.g., "Mid-Term Plan 'Growth Forward' (FY2024-FY2026)") with source references [SSX].
    *   Detail the main objectives and specific quantitative targets (financial and non-financial) outlined in the MTP. Present **all** stated MTP targets/KPIs clearly in a **perfectly formatted Markdown table**, including KPI category, KPI name, target value (with currency/units), target year/period, and baseline values if available [SSX]. Verify data accuracy. Use '-' for missing data points only if needed for table structure.
    ...
    *   Discuss key differences or areas of emphasis compared to the previous MTP, supported by specific examples and inline citations [SSX].

## 3. Strategic Focus Areas and Initiatives within MTP:
    *   For each major strategic pillar identified in the MTP:
...
## 4. Execution, Progress Tracking, and Adaptation:
    *   Identify key internal and external challenges or risks acknowledged by management that affect MTP execution (e.g., "Supply chain disruptions [SSX]", "Talent acquisition difficulties [SSY]").
    *   Describe the specific countermeasures or adjustments stated by the company to address these challenges [SSZ].
    *   Provide the latest available progress updates against the MTP targets (from Section 2 table). Present progress in a **perfectly formatted Markdown table** showing KPI, Target, and Latest Actual/Forecast (with date) [SSX]. Verify data accuracy. Use '-' for missing data points only if needed for table structure.
    ...
    *   Highlight any significant strategic adjustments or MTP revisions announced in response to performance or external events (e.g., "Revised revenue target downwards in Q2 FYYYYY due to market slowdown [SSX]"), with inline citations [SSY].

## 5. General Discussion:
    *   Provide a single concluding paragraph (300-500 words) that synthesizes the key findings from Sections 1-4. Clearly connect each analytical insight with inline citations (e.g., "The strategic focus on DX [SSX] aligns with the MTP targets [SSY], although execution progress shows challenges in margin improvement [SSZ]..."). Explain *why* progress is as reported, based on the analysis. Incorporate key quantitative points.
    ...
    *   Do not introduce any new claims that are not derived from the previous sections and citations.

Source and Accuracy Requirements:
*   **Accuracy:** Information must be factually correct and current. Specify currency and exact dates/periods for all data, targets, and progress reports. Silently omit unverified data after exhaustive search. Verify table data meticulously. Ensure all stated MTP KPIs are captured.
*   **Traceability:** Every claim (in text, tables) must have an inline citation [SSX] linked to the final Sources list.
*   **Source Quality:** Use primarily official company sources (MTP documents, IR presentations, Annual Reports, financial results briefings) with clear and verifiable references.
...
# Updated: Regulatory prompt significantly expanded based on JAL feedback.
def get_regulatory_prompt(company_name: str, language: str = "Japanese"):
    """Generates a prompt for analyzing the regulatory environment with all enhancements."""
    prompt = f'''
...
Target Audience Context: The output is for a **Japanese company** reviewing regulatory risks for potential partnership, investment, or competitive evaluation. Provide exact law/regulation names, dates, reporting periods, and detailed official source references [SSX]. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}
...
{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION} # Focus on official statements and grounded regulatory info
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
{ANALYSIS_SYNTHESIS_INSTRUCTION}
{ADDITIONAL_REFINED_INSTRUCTIONS}
...
### 1. Key Laws, Regulations, and Systems
    *   **Major Applicable Laws/Regulations:** Identify major laws, ordinances, and ministerial regulations related to {company_name}'s industry and operations (e.g., Pharmaceuticals and Medical Devices Act, Building Standards Act, Telecommunications Business Act, Financial Instruments and Exchange Act, sector-specific environmental laws) [SSX]. Specify jurisdiction (e.g., Japan, EU).
    *   **Government/Agency Guidelines & Standards:** Mention key relevant guidelines or standards issued by government bodies or agencies (e.g., METI's Green Growth Strategy Guidelines, specific cybersecurity frameworks referenced) [SSY].
    *   **Potential Legal Amendments:** Discuss any significant upcoming or recent legal amendments mentioned by the company or in grounded sources that could affect operations (immediate to long term) [SSZ].
...
### 2. Licensing and Registration Systems
    ...
    *   **Acquisition & Renewal:** Comment on the perceived difficulty or cost of obtaining/maintaining these licenses, and any recent changes in renewal frequency or examination criteria, if discussed in sources [SSY].
...
### 3. Supervisory Authorities and Industry Influence
    ...
    *   **Industry Associations:** Name key industry associations {company_name}'s part of that issue regulations, policies, or exert lobbying influence [SSY]. Discuss the influence of these associations if commented upon in sources [SSZ].
...
### 4. Market and Business Model Impact
    *   **Competitive Environment Impact:**
        ...
        *   Discuss any changes in competitive structure due to deregulation or stricter enforcement mentioned in sources [SSY].
        *   Note any competitive advantages derived from legislation (e.g., eligibility for specific subsidies or contracts) [SSZ].
    *   **Business Model Impact:**
        *   Detail key regulatory obligations (e.g., information disclosure, audit compliance, reporting requirements like ESG disclosures) [SSX].
        *   Identify regulatory restrictions impacting the business model (e.g., price controls, advertising restrictions, data usage limitations) [SSY].
        *   Discuss the costs and risks associated with compliance [SSZ].

### 5. International Context (if applicable)
    *   **Comparison for Overseas Expansion:** If {company_name} operates internationally or plans expansion, highlight key differences in regulations compared to major overseas markets (e.g., EU regulations, US laws relevant to the industry) based on source information [SSX].
    *   **International Standards & Certifications:** Note compliance status with international standards or certifications relevant to regulation (e.g., ISO standards, GDPR compliance statements, CE Mark for products) [SSY].
    *   **Trade Regulations:** Mention regulations or customs clearance systems related to imports and exports relevant to the business, if discussed [SSZ].

### 6. Recent Policy Trends & Developments
    *   **Latest Trends:** Summarize the latest trends in relevant policies, laws, and regulations mentioned by the company or in grounded sources [SSX].
    *   **Specific Government Measures:** Detail relevant government initiatives like green policies (subsidies, carbon pricing), DX-related legislation, or support programs [SSY].
    ...
    *   **Social Pressure & Activism:** Mention any significant impact from social pressure or citizen/environmental group activism pushing for stricter regulations or specific corporate actions, if documented [SSX].
...
### 7. Compliance Approach & History
    ...
    *   Identify any significant publicly reported regulatory enforcement actions, fines, or controversies related to its operations (not just digital) in the last 3-5 years. Specify dates, regulatory bodies involved, outcomes (including fine amounts with currency), and company responses or remedial actions taken [SSY, SSZ]. Present clearly.

## 8. General Discussion:
    *   Provide a concluding single paragraph (300-500 words) that synthesizes the findings from Sections 1-7. Clearly articulate the primary regulatory pressures {company_name} faces and assess its apparent compliance posture and risk management effectiveness, using inline citations [SSX, SSY].
    ...
    *   Do not introduce new factual claims beyond the provided analysis and citations.

Source and Accuracy Requirements:
*   **Accuracy:** All regulatory details must be current and verifiable. Include specific law names, dates, certification details, and currency information for fines. Silently omit unverified data after exhaustive search.
*   **Traceability:** Each statement must have an inline citation [SSX] corresponding to the final Sources list.
*   **Source Quality:** Use official company disclosures (Annual Reports, Sustainability/ESG Reports, Governance sections, specific policy documents if available), government regulatory websites, and reputable news sources only if grounded by Vertex AI Search results.
...
# Updated: Crisis prompt includes forecasting/DX solution suggestion element.
...
Target Audience Context: This output is for a **Japanese company** assessing digital risk resilience for strategic decision-making. Provide precise data (with dates and reporting periods) and official source references [SSX]. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}
...
{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION} # Focus on official statements + grounded incident reports
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
{ANALYSIS_SYNTHESIS_INSTRUCTION}
{ADDITIONAL_REFINED_INSTRUCTIONS}
...
## 1. Crisis Management and Business Continuity:
    *   **Handling of Past Digital Crises (Last 5 Years):** Describe significant publicly reported digital crises impacting {company_name}. Use bullet points for each incident:
        ...
        *   **Company Response:** Detail the company's public statements, communication strategy, remedial actions taken (e.g., systems restored by [Date] [SSX], external cybersecurity experts engaged [SSY], free credit monitoring offered [SSZ]), and any reported changes to security practices or governance resulting from the incident [SSW].
        *   **Lessons Learned (if stated):** Include any officially stated lessons learned or future preventative measures mentioned by the company [SSX].
    *   **Stated Preparedness and Planning:**
        *   Explain the company's stated approach to digital crisis management. Mention existence of an Incident Response Plan (IRP), Cyber Incident Response Team (CIRT), or similar structures if documented [SSX].
        *   Describe the stated approach to Business Continuity Planning (BCP) specifically for digital operations. Mention existence of BCP documents, disaster recovery (DR) sites, recovery time objectives (RTOs) or recovery point objectives (RPOs) if disclosed [SSY].
        *   Outline the governance structure involved in overseeing digital risk, crisis management, and BCP (e.g., Board committee oversight [SSX], role of CISO/CIO [SSY]). Cite specific sources.
        *   Mention any regular drills, simulations, or third-party audits related to crisis response or BCP, if disclosed [SSZ].
    *   **Risk Forecasting & DX Mitigation (Analysis):**
        *   Discuss any forward-looking risk assessments or forecasting of potential future crisis impacts (e.g., natural disasters affecting data centers, major supply chain digital disruptions) mentioned in company reports (e.g., risk factors section) [SSX].
        *   Based on the identified risks [SSX] or past incident types [SSY], analyze and propose relevant Digital Transformation (DX) based solutions or mitigation strategies that could enhance resilience (e.g., "Given the stated risk of seismic activity near HQ [SSX], DX solutions like geographically distributed cloud backups and enhanced remote work capabilities could mitigate operational disruption."). *This analysis should logically connect identified risks to known DX capabilities.*

## 2. General Discussion:
    *   Provide a concluding single paragraph (300-500 words) synthesizing the findings from Section 1. Assess {company_name}'s apparent resilience to digital disruptions based on its history of incidents, responses, stated preparedness, and the potential application of DX for mitigation. Use inline citations explicitly (e.g., "The company's response to the YYYY incident [SSX] suggests an established protocol, though the stated RTO [SSY] raises questions... The identified risk of X [SSZ] could potentially be addressed by DX initiatives focused on Y...").
    *   Structure the discussion logically, starting with a summary of the incident history and response effectiveness, followed by an evaluation of the stated preparedness measures (IRP, BCP) and risk awareness, incorporating the potential role of DX, and concluding with an assessment of overall digital resilience, identifying potential strengths and weaknesses relevant to a Japanese audience considering partnership or investment.
    *   Do not introduce any new claims not supported by the previous analysis and citations.

Source and Accuracy Requirements:
*   **Accuracy:** All incident details, dates, financial impacts (with currency), and response measures must be current and verifiable against grounded sources. Silently omit unverified data after exhaustive search. Proposed DX solutions should be logical extensions of identified risks/tech capabilities.
*   **Traceability:** Every factual claim must include an inline citation [SSX] linked to a source in the final Sources list.
*   **Source Quality:** Prioritize official company disclosures (press releases on incidents, security sections in reports). Use reputable news or cybersecurity firm reports *only* if grounded by Vertex AI Search results.
...
# Updated: DX prompt includes analysis of the 'why' behind the strategy.
...
Target Audience Context: The analysis is prepared for a **Japanese company** assessing {company_name}'s digital maturity and strategy. Therefore, it must be detailed, with exact figures (specifying currency and reporting periods) and official source references [SSX]. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}
...
{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION} # Focus on DX strategy documents, IR materials
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
{ANALYSIS_SYNTHESIS_INSTRUCTION}
...
## 1. DX Strategy Overview:
    ...
    *   **Analyze the Rationale:** Based on management commentary or strategic documents, explain the *reasons* behind the DX strategy (e.g., "What business problems is DX aiming to solve? How does it link to competitive pressures or the overall corporate vision?") [SSX, SSY].
    *   Identify the key strategic priorities or pillars of the DX strategy (e.g., "Cloud Migration", "AI & Analytics adoption", "Workforce Digital Upskilling", "Supply Chain Optimization") with specific details and start/end dates if part of a formal plan [SSX].
    *   List major DX initiatives or projects currently underway or recently completed under these pillars. Include specific objectives and target outcomes for each initiative if stated (e.g., "Project Phoenix: Cloud migration targeting X% cost reduction by YYYY [SSX]") [SSY].
...
## 2. DX Investments Analysis (Last 3 Fiscal Years):
    ...
    *   Describe overall investment trends in DX over the last 3 years (e.g., increasing significantly [SSX], stable focus on specific areas [SSY]) with supporting data and analysis of the investment allocation strategy [SSZ].

## 3. DX Case Studies & Implementation Examples:
    *   Provide detailed descriptions of 2-3 specific DX implementation examples or case studies highlighted by the company. For each example, describe:
        ...
        *   **Measurable Outcomes & Business Impact:** Quantify results where possible (e.g., "Achieved 12% improvement in OEE in Plant A [SSX]", "Reduced manual reporting time by X hours/week [SSY]", "Enabled new service generating ¥Z million in first year [SSZ]"). Specify currency and reporting period. Use only company-reported outcomes.
        *   **Rationale for Highlighting:** Explain why this example was likely chosen by the company (e.g., flagship project demonstrating AI capability [SSX], successful cross-functional collaboration [SSY]).
...
## 4. Regulatory Environment, Compliance, and Crisis Management (Integration with DX):
    ...
    *   Mention how digital crisis management and business continuity considerations are addressed within the context of major DX initiatives (e.g., "Disaster recovery plans tested for new cloud platform [SSX]", "Redundancy built into critical digital infrastructure [SSY]"). Cite official examples where available [SSZ].

## 5. General Discussion:
    *   Provide a concluding single paragraph (300-500 words) that synthesizes the findings from Sections 1-4. Assess the coherence, ambition, rationale, and execution progress of {company_name}'s DX strategy. Explicitly link data points and examples using inline citations (e.g., "The strategic rationale focusing on customer experience [SSX] drives the significant investment in CRM [SSY], and early results from case studies [SSZ] suggest potential, though scaling remains a challenge...").
    ...
    *   Do not introduce new facts outside of the presented analysis and citations.

Source and Accuracy Requirements:
*   **Accuracy:** All data must be current and verified. Specify currency and reporting period for every monetary value, investment figure, and outcome metric. Silently omit unverified data after exhaustive search. Verify table data meticulously.
*   **Traceability:** Every fact must include an inline citation [SSX] that corresponds to a source in the final Sources list.
*   **Source Quality:** Prioritize official company disclosures (Annual Reports, IR presentations, specific DX reports/webpages) and reputable research *only if grounded* by Vertex AI Search results.
...
# Updated: Business Structure prompt includes check for alternative segmentations/metrics.
def get_business_structure_prompt(company_name: str, language: str = "Japanese"):
    ...
    *   **Priority Information:** Strive to provide, based on exhaustive search of verifiable sources:
    ...
    *   **Check for Alternative Metrics:** If standard revenue segmentation is not the primary method used by the company (e.g., in MTP targets, core reporting for industries like insurance), look for and report the segmentation based on the key metric the company uses (e.g., premiums in-force, assets under management). Clearly define the metric used based on the source.
    *   **Partial Data Handling:** If only partial data (e.g., 1-2 years instead of 3) is available for segments/geography after exhaustive search, present the available data clearly in the tables, noting the timeframe covered (e.g., in the text analyzing the table: "Data for FY2022-2023 shows..." [SSX]). Do not state unavailability. Proceed with analysis based on the available timeframe.

    *   **Verification:** Before completing each section, internally verify:
        * All priority information points are addressed using available grounded data.
...
## 2. Geographic Segment Analysis (Last 3 Fiscal Years):
    *   List the geographic regions or segments as reported by the company (e.g., Japan, North America, Europe, Asia). Identify the primary metric used for geographic segmentation. Include a **perfectly formatted Markdown table** with corresponding figures (specify metric, currency, fiscal year) and composition ratios (%), ensuring totals sum correctly if verifiable [SSX]. Verify data accuracy. Use '-' for missing data points only if needed for table structure.
        ...
        *(Note: Clearly label the 'Metric Value' column header with the specific metric used, e.g., "FYXXXX Revenue (JPY M)" or "FYXXXX Premiums In-Force (JPY Bn)")*
    *   Analyze regional trends (growth/decline YoY calculated from table data, changes in contribution) with specific supporting data [SSX]. Identify key growth markets and declining markets with specific figures [SSY]. Note any stated plans for geographic expansion or contraction with dates and details mentioned in reports [SSZ].

## 3. Major Shareholders & Ownership Structure:
    *   Describe the overall ownership type (e.g., publicly traded on TSE Prime [SSX], privately held) with specific details [SSY].
    *   List the top 5-10 major shareholders in a **perfectly formatted Markdown table** with exact names (as reported, e.g., trust banks), precise ownership percentages, shareholder type (institutional, individual, government, etc.), and the 'as of' date for the data [SSX]. Note any significant changes in top holders over the past year if reported [SSY]. Verify data. Use '-' for missing data points only if needed for table structure.
    ...
    *   Comment briefly on ownership concentration and potential implications (e.g., high institutional ownership suggests focus on governance [SSX], stable founder ownership may influence long-term strategy [SSY]).

## 4. Corporate Group Structure:
    *   Describe the parent-subsidiary relationships and overall corporate group structure based on official filings or reports (e.g., list of major subsidiaries in Annual Report Appendix [SSX]). Note existence/location of group structure charts if found [SSY].
    *   List key operating subsidiaries in a **perfectly formatted Markdown table**, including their official names, primary business functions/segments they operate in, country/region of incorporation, and ownership percentage by the parent company (if stated) [SSX]. Verify data. Use '-' for missing data points only if needed for table structure.
        ...
        | ...                         | ...                                 | ...            | ...         |           |
    *   Note any recent major restructuring, mergers, divestitures, or acquisitions impacting the group structure, providing specific dates and transaction details if available and material [SSY].

## 5. Leadership Strategic Outlook & Vision (Verbatim Quotes - Linkages):
    *   Provide verbatim quotes from key executives (CEO, Chairman, and optionally CFO/CSO) that specifically address:
...
## 6. General Discussion:
    *   Provide a single concluding paragraph (300-500 words) synthesizing the findings from Sections 1-5. Clearly link analytical insights and comparisons using inline citations (e.g., "The shift in segment focus towards Segment B [SSX] aligns with the CEO's stated focus [SSY], but geographic concentration in Japan [SSZ] remains a key factor influencing growth prospects..."). Incorporate key quantitative points.
    *   Address specifically:
        * The alignment (or misalignment) between the business/geographic structure (using the relevant metric) and the stated strategic vision/MTP [SSX, SSY].
        * How the ownership structure may influence business decisions or governance [SSZ].
        * Key opportunities or challenges presented by the current segment mix and geographic footprint [SSW].
        * Potential future developments or necessary structural changes based on current structure, trends, and leadership comments [SSX, SSY].
...
Source and Accuracy Requirements:
*   **Accuracy:** Ensure all data is precise, with currency and fiscal year reported for numerical values. Use official names for segments, regions, shareholders, and subsidiaries. Use the correct segmentation metric as reported by the company. Silently omit unverified data after exhaustive search. Verify table data meticulously.
*   **Traceability:** Every fact (in text, tables) must include an inline citation [SSX] corresponding to the final Sources list.
*   **Source Quality:** Use only primary official sources (Annual/Integrated Reports, Financial Statements, Filings, IR Presentations, Governance Reports) with clear documentation references.
...
# Updated: Vision prompt structure maintained, relying on general instruction updates.
...
## 1. Company Vision and Strategy Elements:
    ...
    *   **Strategic Vision Components/Pillars:** List and explain the key strategic themes, values, or pillars that underpin the vision (e.g., "Innovation", "Sustainability", "Customer Centricity") as defined in official documents [SSX]. Provide brief definitions or explanations for each pillar based on the source [SSY].
    *   **Vision Measures / KPIs:** Identify specific measures or Key Performance Indicators (KPIs) that the company explicitly links to tracking progress towards its overall vision or purpose (these might be high-level MTP targets or specific ESG goals mentioned in the vision context). Present these in a list or **perfectly formatted Markdown table** if multiple and verifiable, including the KPI name, the target (if specified, with date/period), and how it relates to the vision pillar [SSX]. Verify data. Use '-' for missing data points only if needed for table structure.
        ...
        | ...                | ...                           | ...                            |           |
    *   ***Stakeholder Focus:*** Analyze how the vision statement and its supporting pillars explicitly address or prioritize key stakeholder groups (e.g., customers, employees, shareholders, society, environment) based on the language used in official communications [SSX]. Provide specific examples or quotes [SSY].

## 2. General Discussion:
    *   Provide a concluding single paragraph (300-500 words) synthesizing the information in Section 1. Evaluate the clarity, ambition, distinctiveness, and internal coherence of the stated vision and its components. Use inline citations to link back to specific elements (e.g., "The vision's focus on sustainability [SSX] is clearly measured by the CO2 reduction KPI [SSY], demonstrating commitment... However, the link between the 'Innovation' pillar and specific KPIs appears less defined [SSZ] based on available public disclosures..."). Incorporate key quantitative points if available.
    ...
    *   Do not introduce new claims beyond the synthesized findings from Section 1 and citations.

Source and Accuracy Requirements:
*   **Accuracy:** Ensure all statements, quotes, and KPIs are accurately represented from official sources and current as of the cited document date. Specify currency/units for KPIs where applicable. Silently omit unverified data after exhaustive search. Verify table data.
*   **Traceability:** Every claim must have an inline citation [SSX] that corresponds to a source in the final Sources list.
*   **Source Quality:** Use primarily official company documents (Integrated Reports, dedicated Vision/Purpose web pages, MTP overviews, Sustainability Reports) and well-documented press releases related to strategy announcements.
...
# Updated: Management Message prompt explicitly requests quotes from other relevant CxOs and incorporates general instruction enhancements.
...
### [Chairman Full Name], [Chairman Title] (if distinct from CEO and provides verifiable strategic commentary)
...
### [Other Key Executive Name], [Title] (e.g., CFO, CSO, CTO, COO, BU Head - Include significant, verifiable strategic quotes)
...
## 2. General Discussion:
    ...
    *   Do not introduce any new factual claims or analysis beyond what is directly supported by the quotes provided and cited.

Source and Accuracy Requirements:
*   **Accuracy:** Every quote must be verbatim, correctly attributed to the speaker (with title), and include precise source details (document/event, date, page/time if possible). Silently omit quotes if not verifiable after exhaustive search.
*   **Traceability:** Each quote's origin must be confirmed by an inline citation [SSX] corresponding to the final Sources list.
*   **Source Quality:** Use only official communications (Annual/Integrated reports, earnings call transcripts, official IR presentations/webcasts, company-published interviews). Avoid secondary reporting of quotes unless the secondary source itself is grounded.
...
# Updated: Account Strategy prompt benefits from general instruction enhancements.
def get_account_strategy_prompt(company_name: str, language: str = "Japanese", context_company_name: str = "NESIC"):
    """
    ...
    inline citation [SSX]. Use **perfect Markdown tables**.
    """
    nesic_context_note = textwrap.dedent(f"""\
    **{context_company_name} Context Integration Note:**
    *   While the objective is to create a plan *for* {context_company_name}, this prompt focuses on gathering verifiable information *about* **{company_name}** using Gemini grounding.
...
Objective: Create a detailed, data-driven account strategy action plan for {company_name} covering the next three fiscal years (e.g., FY2025-FY2027). This plan must be based *exclusively* on verifiable information about {company_name} obtained through grounded sources. The analysis should identify opportunities where known {context_company_name} capabilities could align with {company_name}'s stated needs or challenges. Focus strictly on {company_name}.

Target Audience Context: This plan is for internal use by {context_company_name} sales and strategy teams. Recommendations should highlight potential alignments between {company_name}'s verified situation [SSX] and {context_company_name}'s publicly known core competencies. {AUDIENCE_CONTEXT_REMINDER.format(language=language)}

{get_language_instruction(language)}
{nesic_context_note}

Research Requirements:
*   Use only data about {company_name} validated through Gemini grounding URLs [SSX]. Perform exhaustive checks across multiple sources (including latest reports/filings) before silently omitting unverified data.
*   Every fact, figure, or stated initiative of {company_name} must be backed by an inline citation [SSX]. Silently omit any unverified points about {company_name}.
*   Use **perfect Markdown tables**. Verify data accuracy. Use '-' for missing data points only if needed for structure.
*   Focus on extracting actionable intelligence about {company_name} that informs strategic engagement by {context_company_name}.

{HANDLING_MISSING_INFO_INSTRUCTION}
{RESEARCH_DEPTH_INSTRUCTION} # Focus on {company_name}'s reports, financials, strategy docs
{SPECIFICITY_INSTRUCTION}
{INLINE_CITATION_INSTRUCTION}
{ANALYSIS_SYNTHESIS_INSTRUCTION} # Focus analysis on identifying needs {context_company_name} could meet

{ADDITIONAL_REFINED_INSTRUCTIONS}

## 1. Customer Profile ({company_name})
    *   **Company Name:** {company_name} [SSX]
    *   **Industry:** [Primary Industry Classification] [SSX]
    ...
    *   **Core Business Summary:** Briefly summarize main operations and markets based on latest reports [SSX].
    *   *(Note: Avoid speculation on {context_company_name} relationship unless verifiable [SSY].)*

## 2. Revenue Analysis & Growth Drivers ({company_name})
    *   Present Revenue for last 3 fiscal years (FYXXXX, FYYYYY, FYZZZZ) in a **perfectly formatted Markdown table**, specifying currency (e.g., JPY Millions) [SSX]. Calculate YoY Growth Rate (%). Verify data. Use '-' for missing data points only if needed for table structure.
        | Metric                  | FYXXXX | FYYYYY | FYZZZZ | Source(s) |
        |-------------------------|--------|--------|--------|-----------|
        | Total Revenue (JPY M)   |        |        |        | [SSX]     |
        | YoY Growth Rate (%)     | N/A    |  X.X%  |  Y.Y%  | (Calc)    |
    *   Identify key business segments or geographic regions driving revenue growth or decline, based on sourced segment data [SSY]. Use latest available data.
...
## 3. Financial Performance Indicators ({company_name})
    *   Present Net Income (Attributable to Parent) and Operating Margin (%) for the last 3 fiscal years in a **perfectly formatted Markdown table** [SSX]. Verify data. Use '-' for missing data points only if needed for table structure.
        | Metric                           | FYXXXX | FYYYYY | FYZZZZ | Source(s) |
        |----------------------------------|--------|--------|--------|-----------|
        | Net Income (Parent) (JPY M)      |        |        |        | [SSX]     |
        | Operating Margin (%)             | -      |        |        | [SSY]     |
    *   Note key profitable divisions/segments if identifiable from sourced data [SSZ].
    *   Analyze implications: Does financial health suggest capacity for large projects? Are margin pressures creating a need for cost optimization solutions {context_company_name} could provide? [SSX, SSY, SSZ]
...
## 4. Strategic Initiatives & Potential {context_company_name} Alignments ({company_name})
    *   List {company_name}'s major publicly stated strategic initiatives for the next 1-3 years (from latest MTP, Annual Report, IR presentations). Include focus areas (e.g., DX, ESG, Global Expansion, R&D), specific goals, timelines, and investment amounts (with currency) if available [SSX]. Use bullet points:
        *   **Initiative 1:** [Name/Focus, e.g., "Digital Transformation Program 'NextGen'"] [SSX]
        ...
        *   *(List 3-5 major verifiable initiatives from latest sources)*
    *   For each verifiable initiative, explicitly state how known {context_company_name} capabilities could support {company_name}'s stated goals.
...
## 5. Decision-Making Structure & Stakeholders ({company_name})
    ...
    *   Identify key executives (names, titles) responsible for strategy, IT, operations, or relevant business units, based on the latest verifiable management structure information [SSY]. Verify titles.
    *   *(Note: Avoid listing specific contacts unless publicly available in directories and verifiable via grounding.)*
...
## 6. Critical Business Challenges & Potential {context_company_name} Solutions ({company_name})
    *   Enumerate {company_name}'s major challenges as explicitly mentioned in official sources (e.g., Annual Report risk section, CEO statements). Categorize if possible (Operational, Technological, Market, Strategic) [SSX].
        ...
        *   *(List 3-5 key verifiable challenges from latest sources)*
    *   For each verifiable challenge, propose specific, relevant {context_company_name} service categories (based on public knowledge) that directly address the verified problem.
...
## 7. Technology Roadmap & {context_company_name} Capabilities ({company_name})
    ...
    *   Map these verifiable focus areas to {context_company_name}'s known public service portfolio (e.g., Multi-cloud integration services, Data analytics platform support, Secure IoT network solutions, Managed network services).
...
## 8. Engagement Strategy Outline (FY2025–2027)
    *   Provide a high-level quarterly engagement plan concept for {context_company_name}. Focus on *themes* derived from {company_name}'s verified needs, not specific deal values unless grounded. Use a **perfectly formatted Markdown table**. Verify data links. Use '-' for missing data points only if needed for structure.
        | Fiscal Year / Quarter | Engagement Theme / Focus Area (based on {company_name}'s needs) | Potential {context_company_name} Services to Discuss                    | Target Dept/Stakeholder (Concept) | Supporting {company_name} Data Point |
        |-----------------------|-------------------------------------------------------------------|--------------------------------------------------------|-----------------------------------|--------------------------------------|
    ...
    *   Ensure each proposed engagement theme directly links back to a verified need, challenge, or initiative identified in earlier sections [SSX, SSY, SSZ].
...
## 10. Success Metrics & Potential KPIs for {context_company_name}
    *   Define 3-5 specific, measurable Key Performance Indicators (KPIs) for {context_company_name}'s engagement with {company_name} over the 3 years. These should be *internal* {context_company_name} goals based on the opportunities identified through verifiable data about {company_name}.
        ...
        *   Example KPI 3: Penetration of {context_company_name} services into new departments/business units (Target: Enter Z new BUs by FY2027).
