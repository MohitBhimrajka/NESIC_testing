...
def get_account_strategy_prompt(company_name: str, language: str = "Japanese", ticker: Optional[str] = None, industry: Optional[str] = None, context_company_name: str = "NESIC"):
    ...
    *   Any claims about past {context_company_name}-{company_name} interactions require separate verifiable sources [SSY] and should only be included if grounded. Silently omit if not verifiable.
    ...
    prompt = f"""
**CRITICAL FOCUS:** This entire request is *exclusively* about the specific entity: {context_str}. Verify the identity of the company for all sourced information. Do not include unrelated entities. This plan leverages public data about {company_name} to inform strategy for {context_company_name}.
...
## 1. Customer Profile ({company_name})
    ...
    *   **Core Business Summary:** Briefly summarize main operations and markets based on latest reports for {company_name} [SSX].
...
## 2. Revenue Analysis & Growth Drivers ({company_name})
    *   Present Revenue for {company_name} for last 3 fiscal years (FYXXXX, FYYYYY, FYZZZZ) in a **perfectly formatted Markdown table**, specifying currency (e.g., JPY Millions) [SSX]. Calculate YoY Growth Rate (%). Verify data. Use '-' for missing data points only if needed for table structure.
...
## 5. Decision-Making Structure & Stakeholders ({company_name})
    ...
    *   Identify key executives (names, titles) within {company_name} responsible for strategy, IT, operations, or relevant business units, based on the latest verifiable management structure information [SSY]. Verify titles.
